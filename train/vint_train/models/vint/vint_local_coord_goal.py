...
class ViNTLocalCoordGoal(ViNT):
    def __init__(
        ...
        """
        super(ViNTLocalCoordGoal, self).__init__(context_size, len_traj_pred, learn_angle)
        ...
        self.fixed_learnable_goal_embedding = nn.Parameter(torch.randn(1,3000))
        self.local_coord_encoder = nn.Sequential(
            nn.Linear(3002, 512),
            nn.ReLU(),
        ...
        del self.dist_predictor
        
    def forward(
        self, obs_img: torch.tensor, goal_coords: torch.tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:

        
        # get the fused observation and goal encoding
        #if self.late_fusion:
        goal_encoding = self.local_coord_encoder(torch.hstack([self.fixed_learnable_goal_embedding.expand(obs_img.shape[0], -1),goal_coords]))
        #else:
        #    obsgoal_img = torch.cat([obs_img[:, 3*self.context_size:, :, :], goal_coords], dim=1)
        ...
        return (torch.ones(action_pred.shape[0],1)*self.len_trajectory_pred).to(action_pred.device), action_pred