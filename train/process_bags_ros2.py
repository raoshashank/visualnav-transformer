    ...
    for root, _, files in os.walk(args.input_dir):
        for file in files:
            if file.endswith(".db3"):
                ...
                break
    print("Number of bags found: ", len(bag_files))
    if args.num_trajs >= 0:
        bag_files = bag_files[: args.num_trajs]
    print("Number of bags processed: ", len(bag_files))
    # processing loop
    done = []
    try:
        with open(os.path.join(args.output_dir,'done.txt'),'r') as f:
          done = f.read().splitlines()
    except FileNotFoundError:
        print("No previous data found. Starting from scratch.")
    for bag_path in tqdm.tqdm(bag_files, desc="Bags processed"):
        if bag_path.split('/')[-1] in done:
            print(f"{bag_path} already processed. Skipping...")
            continue
        try:
            type_map = {
                config[args.dataset_name]["odomtopics"]: MESSAGE_TYPE_MAP[config[args.dataset_name]['typemap'][config[args.dataset_name]["odomtopics"]]],
                config[args.dataset_name]["imtopics"]: MESSAGE_TYPE_MAP[config[args.dataset_name]['typemap'][config[args.dataset_name]["imtopics"]]],
            }
